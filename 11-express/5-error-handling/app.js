...
app.get("/file3", async (req, res) => {
  try {
    ...
    // 해당 부분은 동기적으로 동작함.
    // 하지만 error 미들웨어까지 도달하지 못함.
    // 왜냐하면 async-await 패턴은 결국 promise 를 반환하기 때문!
    // => 48 라인에서 에러가 발생해도 moddleware error handler 까지 도달하지 못함

    // 단, async 패턴은 promise 를 동기적으로 사용할 수 있게 했으므로
    // try-catch 문으로 에러를 잡을 수 있음
  } catch {
    res.sendStatus(404);
  }
});
